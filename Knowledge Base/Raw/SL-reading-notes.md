...
称 $x\in \mathcal{X}$ 为**预测因子**（predictor）/ **特征**（feature）， $y\in Y$ 为**响应**（response）/ **标签**（target）. 称 $\mathcal{X}$ 为特征空间， $\mathcal{Y}$ 为响应空间.

$f$ 为根据 $x$ 预测 $y$ 提供了**系统信息**（systemmatic information），但仍然存在其他未被观测到的变量会对 $y$ 产生影响，因此假设存在误差项.

假设从$\mathcal{X}$中独立同$X$分布（i.i.d）地抽取了$(x_1,x_2,\cdots,x_N)$，根据上面地假设会观测到相应的$(y_1,y_2,\cdots,y_N)$，于是有**数据集**（data set）：$$T=\{(x_1,y_1),(x_2,y_2),\cdots(x_N,y_N)\}$$称$(x_i,y_i),i=1,2,\cdots,N$为一个**数据点**（data point）.
...
先考虑最简单的情形：假设真实函数为：$$\begin{aligned}
Y &=f(X)+\epsilon\\
&=\alpha_0+\alpha_1X+\epsilon\\
&\text{mean}(\epsilon)=0
\end{aligned}$$并且$X,\epsilon$独立.

下面尝试给出对于$f(X)$的估计：$$\hat{f}(X)=\beta_0+\beta_1X_1$$

其中$\beta_0,\beta_1$称为系数（coefficients）/ 参数（parameters）
...
假设用多个大小相等的数据集 $T$ 训练 $\hat{f}$ ， $\hat{f}$ 中两个参数 $\beta_0,\beta_1$ 可以视为随机变量，现在估量用上述方法估计得到的参数 $\beta_0,\beta_1$ 与 $f$ 中两个参数 $\alpha_0,\alpha_1$ 之间的差距随着数据集大小 $N$ 的变化会如何变化，考虑计算**标准误差**（standard errors）：$$\begin{aligned}
...
&1,\text{if male}\\
&-1,\text{if female}
\end{aligned}\right.$$ 这样一个变量称为**哑变量**（dummy variable）或者**指示变量**（indicators）至于之后取 $\{-1,1\}$ 还是 $\{0,1\}$ 并不重要. #issue %%理论上怎么证明，之前看感知机的时候能看出来是一样的%%
...
统计模型在高维情形下表现明显低于低维的情况称为**维数灾难**（curse of dimensionality）. 对于高维情形，KNN中计算得到的近邻点实际上可能非常远，从而造成很差的拟合效果. 
...
考虑类别只有两种的情况，用 $0,1$ 代替. 定义 $p(X)=\text{Pr}(Y=1|X)$. **逻辑斯蒂函数**（logistic function）定义为： $$p(X)=\frac{e^{\beta_0+\beta_1X}}{1+e^{\beta_0+\beta_1X}}$$

为拟合模型，采取**极大似然估计**（maximum likelihood）. 即为取使得事件最有可能发生的参数： $$(\beta_0,\beta_1)=\arg\max p(Y|X,T)$$ 其中 $T$ 为数据集， $$\begin{aligned}
p(Y|X,T)&=p(y_1,\cdots,y_N|x_1,\cdots,x_N)\\
&=\prod_{i=1}^{N}p(y_i|x_i)\\
&=\prod_{i=1}^{N}\frac{e^{\beta_0+\beta_1x_i}}{1+e^{\beta_0+\beta_1x_i}}
\end{aligned}$$
...
logistic回归也有较强的可解释性，对于上面的赔率，观察到 $X$ 增加一个单位，赔率将增加 $\beta_1$ 个单位，而logit则乘以 $e^{\beta_1}$ 个单位.
